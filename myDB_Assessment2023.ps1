...
function AzureDBDMA {
    ...
    #)
     #$msgBoxInput =  [System.Windows.MessageBox]::Show('Would you like to continue ?','Option  input','YesNoCancel','Error')

    #switch  ($msgBoxInput) {

    #'Yes' {
  Add-Type -AssemblyName PresentationFramework
  Write-Host "AzureDB function" -ForegroundColor Yellow
  clear-host
    $Target = $the_combo.text
  #Remove-Item "C:\DMA_Reports_Final_named" -Force -Recurse
  #$DMA_Path = "C:\DMA_PowerShell_WindowsGUI\"
  $DMA_Path =$Dir_Path#"C:\DMA_Report_"+$(get-date -f dd-MM-yyyy)+'\'
  $TargetPlatform=$Target#"AzureSqlDatabase"
  #validate directory 
  if(!(Test-Path -Path $DMA_Path)) 
        {
      #New-Item "$InstallDir" -type directory -Force | out-null
            new-item -type directory -path $DMA_Path -Force
            new-item -type file -path $DMA_Path"MS_Instancelist.txt" -Force
            new-item -type file -path $DMA_Path"Serverlist.txt" -Force
            new-item -type file -path $DMA_Path"server_not_connect.txt" -Force
            new-item -type file -path $DMA_Path"Assess-for-AzureSQLMI.xml" -Force
            Start-Sleep 5
            write-host "Content adding on XML file " -ForegroundColor Yellow
            #Add-Content $DMA_Path"MS_Instancelist.txt" "`nsqlserver01"
            
            Add-Content $DMA_Path"Assess-for-AzureSQLMI.xml" -Value '<?xml version="1.0" encoding="UTF-8"?>
      <AssessmentConfiguration xmlns="http://microsoft.com/schemas/SqlServer/Advisor/AssessmentConfiguration">
      <AssessmentName>Scale-Assessment-for-AzureSQLManagedInstance</AssessmentName>
      <AssessmentSourcePlatform>SqlOnPrem</AssessmentSourcePlatform>
      <AssessmentTargetPlatform>ManagedSqlServer</AssessmentTargetPlatform>
    ...
    write-host "Content added on XML file " -ForegroundColor Green
    write-host "Directory and file created sucessfully , Please enter server name on file $DMA_Path"MS_Instancelist.txt" and run again same code" -ForegroundColor Yellow
    start-sleep 10
    #exit;
    [void] [System.Windows.MessageBox]::Show( "Directory $DMA_Path Created,Please enter Server name ", "Script completed", "OK", "Information" )
    #$mainForm.Close()
    #$mainForm.Dispose()
    #$mainForm.MenuComplete
    #[void]$mainForm.FormClosed
    ...
    #AllSQLServer
      ...
      {
        write-host "Folder $DMA_Path already exists" -ForegroundColor Green
        #[void] [System.Windows.MessageBox]::Show( "Directory $DMA_Path Already exist : Are you sure want to run DMA Report on same servers or update new", "Script completed", "OK", "Information" )
        #[void]$mainFormSI.ShowDialog()
        #AllSQLServer
      #}
    #$servername = $Null
    [System.Reflection.Assembly]::LoadWithPartialName('Microsoft.SqlServer.SMO')
    $sucess=$DMA_Path+"server_connect.txt"
    $failure=$DMA_Path+"server_not_connect.txt"
    $srvoutput=$DMA_Path+"MS_Instancelist.txt"
    $ServerNames = get-content $DMA_Path"serverlist.txt"
    #$DatabaseName="master"
    $s=$null
    #$server = Get-Content -path "C:\scripts\ServerList.txt"
    #C:\Users\Al1014368\Documents\MS_Instancelist.txt
    #[System.Reflection.Assembly]::LoadWithPartialName('Microsoft.SqlServer.SMO')    
...

    {
      TRY
      {
        ...
        
        {
          ...
          #$p=$Null
          #$p = ((reg query "\\$s\HKLM\SOFTWARE\Microsoft\Microsoft SQL Server\instance names\sql" -v $i) -replace "    $i    REG_SZ    ")
          ...
          if($i -eq 'MSSQLSERVER')
          {
            ...
            #$s  
            #$a="C:\myDB_Assessment_Report\DMA\"     
            $path=new-item -type directory -path $DMA_Path"DMA\"$srv -Force 
            #$path
            $new = $DMA_Path+"DMA\"+$srv
            $new
            #write-host "Checking server: $servername" -ForegroundColor Magenta
            $xmlfilepath = $DMA_Path+"Assess-for-AzureSQLMI.xml"
            [xml]$myxml = get-content $xmlfilepath
            $myxml.AssessmentConfiguration.AssessmentName = "Scale-Assessment-for-AzureSQL"+$TargetPlatform
            $myxml.AssessmentConfiguration.AssessmentTargetPlatform =$TargetPlatform
            $myxml.AssessmentConfiguration.AssessmentResultDma = $new +"\"+ $srv+"_"+$TargetPlatform+".dma"
            $myxml.AssessmentConfiguration.AssessmentResultJson = $new +"\"+ $srv+"_"+$TargetPlatform+".Json"
            $myxml.AssessmentConfiguration.AssessmentResultCsv = $new +"\"+ $srv+"_"+$TargetPlatform+".csv"
            $myxml.AssessmentConfiguration.AssessmentDatabases.AssessmentDatabase = "Server=$srv;Integrated Security=true"
            $myxml.Save("$xmlfilepath")
            $readjsonfile=$new +"\"+ $srv+"_"+$TargetPlatform+".Json"
            start-sleep 3
            write-host "DMA running on server: $srv" -ForegroundColor Magenta
            #write command to run dma.exe command for each server
            $varmain="C:\Program Files\Microsoft Data Migration Assistant\DmaCmd.exe" 
            $var = "/Action=Assess /AssessmentConfiguration= $xmlfilepath"
            $var1= $var.Split(" ")
            & $varmain $var1
            #$mainForm.Close()
            Write-Host "DMA Report completed for Server:$srv" -ForegroundColor Yellow
            #[void] [System.Windows.MessageBox]::Show( "DMA Report created successfully ", "Script completed", "OK", "Information" )
            #start-sleep 3
            #$s=$Null
            Start-Sleep 5
            DMA_Data_Insert $readjsonfile
            ########################
          }
          else{        
            #$name = $s+'\'+$i | Out-File -FilePath C:\DMA_Reports_Final_named\MS_Instancelist.txt -Append 
            ...
            write-host "Nameed instance loop" -ForegroundColor Yellow
              ...
              $d=$srv1.replace('\','_')
            #$d=$s.Split("\")
            $path=new-item -type directory -path $DMA_Path"DMA\"$d -Force 
            $new = $DMA_Path+"DMA\"+$d
            #"3333333333333333333" +$new
            #write-host "Checking server: $servername" -ForegroundColor Magenta
            $xmlfilepath = $DMA_Path+"Assess-for-AzureSQLMI.xml"
            [xml]$myxml = get-content $xmlfilepath
            $myxml.AssessmentConfiguration.AssessmentName = "Scale-Assessment-for-AzureSQL"+$TargetPlatform
            $myxml.AssessmentConfiguration.AssessmentTargetPlatform =$TargetPlatform
            $myxml.AssessmentConfiguration.AssessmentResultDma = $new +"\"+ $d+"_"+$TargetPlatform+".dma"
            $myxml.AssessmentConfiguration.AssessmentResultJson = $new +"\"+ $d+"_"+$TargetPlatform+".Json"
            $myxml.AssessmentConfiguration.AssessmentResultCsv = $new +"\"+ $d+"_"+$TargetPlatform+".csv"
            $myxml.AssessmentConfiguration.AssessmentDatabases.AssessmentDatabase = "Server=$srv1;Integrated Security=true"
            $myxml.Save("$xmlfilepath")
            start-sleep 3
            write-host "DMA running on server: $srv1" -ForegroundColor Magenta
            #write command to run dma.exe command for each server
            $varmain="C:\Program Files\Microsoft Data Migration Assistant\DmaCmd.exe" 
            $var = "/Action=Assess /AssessmentConfiguration= $xmlfilepath"
            $var1= $var.Split(" ")
            & $varmain $var1
            Write-Host "DMA Report completed for Server:$srv1" -ForegroundColor Yellow
            #[void] [System.Windows.MessageBox]::Show( "DMA Report created successfully ", "Script completed", "OK", "Information" )
            start-sleep 3
            #$snamed=$Null
            Start-Sleep 5
            $readjsonfile =$new +"\"+ $d+"_"+$TargetPlatform+".Json"
            DMA_Data_Insert $readjsonfile
            #####################
             }                          
       ...
       #[void] [System.Windows.MessageBox]::Show( "DMA Report created successfully for all PaaS Servers", "Script completed", "OK", "Information" )  
       
    }
    } #else end of alredy exist
    #[void]$mainForm.Close()
    #$statusLabel.Text = "Ready"
    #$mainForm.Close()
    [void] [System.Windows.MessageBox]::Show( "DMA Report created successfully for all the PaaS Servers", "Script completed", "OK", "Information" )
    #[void]$mainForm.Close()
    #$statusLabel.Text = "Ready"
    #exit;
}
...
#Function Single Instance PaaS DMA Report creation Menu
<#
Function DMA-Single-Instance
{
      
        #clear-host

        #Values from SubWindowForm
        $srvname = $textBoxIn.Text
        $Target = $the_combo.text
        $failure=$DMA_Path+'Inventory\'+"server_not_connect.txt"
        $success=$DMA_Path+'Inventory\'+"server_connect.txt"
        write-host "$srvname is Processing.. your request" -ForegroundColor Magenta
        [string]$TargetPlatform=$Target
        
        if(!(Test-Path -Path $DMA_Path)) 
        {
          
          write-host "Directory $DMA_Path not exist " -ForegroundColor Yellow
          [void] [System.Windows.MessageBox]::Show( "Directory $DMA_Path not exist,We can not process ", "Script completed", "OK", "Information" )
          start-sleep 4
          [void]$mainForm.Close()
          break;
        }
        ...
        {
          write-host "Directory $DMA_Path already exists" -ForegroundColor Green
          #Remove-Item $Dir_Path"Inventory"'\'"single_instance.txt" -Force -Recurse #-ErrorAction continue
          #new-item -type file -path $Dir_Path"Inventory"'\'"single_instance.txt"
          #Add-Content $Dir_Path"Inventory"'\'"single_instance.txt" $srvname
          ...
          # Use function to validate Server name and Login to server if exist than only process 
          $result=SQL_SERVER_Validation -s $srvname
          if ($result -eq $False)
            {
               
               write-host "Server $srvname is not connected using validation function" -ForegroundColor red
               [void] [System.Windows.MessageBox]::Show( "$srvname unable to connect,Please check  ", "Script completed", "OK", "Information" )
               #start-sleep 2
              #[void]$mainForm.Close()
              #$mainFormSI.close()
          ...
          $s=$NULL
          $s=$servername.Name
          #$servername.Databases | Select-Object -ExpandProperty name
                
          Write-Host "$s,up" -ForegroundColor Green
          "$s,up  (single Instance PaaS)" | Out-file $success -Append
          if ($servername.ComputerNamePhysicalNetBIOS -ne $srvname) # Named Instance
            {
              write-host "Nameed instance loop" -ForegroundColor Yellow
              $d=$s.replace('\','_')
            #$d=$s.Split("\")
            $path=new-item -type directory -path $DMA_Path"SingleInstance\"$d -Force 
            $new = $DMA_Path+"SingleInstance\"+$d
            #write-host "Checking server: $servername" -ForegroundColor Magenta
            $xmlfilepath = $DMA_Path+'Inventory\'+"Assess-for-AzureSQLMI.xml"
            [xml]$myxml = get-content $xmlfilepath
            $myxml.AssessmentConfiguration.AssessmentName = "Scale-Assessment-for-AzureSQL"+$TargetPlatform
            $myxml.AssessmentConfiguration.AssessmentTargetPlatform =$TargetPlatform
            $myxml.AssessmentConfiguration.AssessmentResultDma = $new +"\"+ $d+"_"+$TargetPlatform+".dma"
            $myxml.AssessmentConfiguration.AssessmentResultJson = $new +"\"+ $d+"_"+$TargetPlatform+".Json"
            $myxml.AssessmentConfiguration.AssessmentResultCsv = $new +"\"+ $d+"_"+$TargetPlatform+".csv"
            $myxml.AssessmentConfiguration.AssessmentDatabases.AssessmentDatabase = "Server=$s;Integrated Security=true"
            $myxml.Save("$xmlfilepath")
            write-host "DMA running on server: $s" -ForegroundColor Magenta
            #write command to run dma.exe command for each server
            $varmain="C:\Program Files\Microsoft Data Migration Assistant\DmaCmd.exe" 
            $var = "/Action=Assess /AssessmentConfiguration= $xmlfilepath"
            $var1= $var.Split(" ")
            & $varmain $var1
            Write-Host "DMA Report for Single Instance PaaS completed for Server:$s" -ForegroundColor Yellow
            start-sleep 3
            $s=$Null
             } 
            ...
            $path=new-item -type directory -path $DMA_Path"SingleInstance\"$s -Force 
            #$path
            $new = $DMA_Path+"SingleInstance\"+$s
            #$new
            #write-host "Checking server: $servername" -ForegroundColor Magenta
            $xmlfilepath = $DMA_Path+'Inventory\'+"Assess-for-AzureSQLMI.xml"
            [xml]$myxml = get-content $xmlfilepath
            $myxml.AssessmentConfiguration.AssessmentName = "Scale-Assessment-for-AzureSQL"+$TargetPlatform
            $myxml.AssessmentConfiguration.AssessmentTargetPlatform =$TargetPlatform
            $myxml.AssessmentConfiguration.AssessmentResultDma = $new +"\"+ $s+"_"+$TargetPlatform+".dma"
            $myxml.AssessmentConfiguration.AssessmentResultJson = $new +"\"+ $s+"_"+$TargetPlatform+".Json"
            $myxml.AssessmentConfiguration.AssessmentResultCsv = $new +"\"+ $s+"_"+$TargetPlatform+".csv"
            $myxml.AssessmentConfiguration.AssessmentDatabases.AssessmentDatabase = "Server=$s;Integrated Security=true"
            $myxml.Save("$xmlfilepath")
            write-host "DMA running on server: $s" -ForegroundColor Magenta
            #write command to run dma.exe command for each server
        ...
        Write-Host "DMA Report of Single Instance PaaS completed For server:$S" -ForegroundColor Yellow
        start-sleep 2
        [void] [System.Windows.MessageBox]::Show( "SQL Server Single Instance PaaS Report created successfully ", "Script completed", "OK", "Information" )
        $srvname=$null
        $TargetPlatform=$null
        $failure=$null
        $success=$null
...
function Import_JSON_to_SQLDB
{
    ...
    if($InstanceNamer -ne "")
    {
        dmaProcessor -serverName $InstanceName `
            -databaseName DMAReporting2023 `
            -jsonDirectory "$InputOutputFolder" `
            -processTo SQLServer 
...
Function DMA_Report_All
{
  ...
  $TargetServerInstance = $env:COMPUTERNAME#"sqlvs001"
...
SELECT [ImportDate]
    ...
    FROM [DMAReporting2023].[dbo].[ReportData] where ChangeCategory in ('BreakingChange','MigrationBlocker','Information')
    )
    SELECT * 
    FROM cte
  ...
  $execq1=Invoke-Sqlcmd -ServerInstance $Servername -Database DMA -Query  $query1 -EA "silentlycontinue" |Export-csv -path $D -NoTypeInformation
...
select InstanceName,Name,
	(select  sum(case when r.ChangeCategory='BreakingChange' and AssessmentTarget='AzureSQLDatabase' and TargetCompatibilityLevel = 'CompatLevel150' then 1 else 0 end)  as ReportBreakinghanges from [DMAReporting2023].[dbo].[ReportData] r where dbinfo.InstanceName=r.InstanceName and r.Name=dbinfo.Name) TotalAzureSQLDBBreakingChanges,
	(select  sum(case when r.ChangeCategory='BreakingChange' and AssessmentTarget='ManagedSQLServer' and TargetCompatibilityLevel = 'CompatLevel150' then 1 else 0 end)  as ReportBreakinghanges from [DMAReporting2023].[dbo].[ReportData] r where dbinfo.InstanceName=r.InstanceName and r.Name=dbinfo.Name) TotalMIBreakingChanges,
	...
	(select  sum(case when r.ChangeCategory='MigrationBlocker' and AssessmentTarget='ManagedSQLServer' and TargetCompatibilityLevel = 'CompatLevel150' then 1 else 0 end)  as ReportBreakinghanges from [DMAReporting2023].[dbo].[ReportData] r where dbinfo.InstanceName=r.InstanceName and r.Name=dbinfo.Name) TotatlMIBlocker,
	...
	from 
	(
	select  InstanceName,Name from [DMAReporting2023].[dbo].[ReportData] group by InstanceName,Name)dbinfo
	)FinalOutput
  "
  #$eqecutequery1= Invoke-SQLCMD $query1  -ServerInstance $SQLServer
